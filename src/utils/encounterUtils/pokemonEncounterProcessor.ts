...
export const deduplicatePokemonLocations = (locations: PokemonLocationData[]): PokemonLocationData[] => {
  ...
  return Array.from(grouped.entries()).map(([, locationGroup]) => {
    const consolidated: PokemonLocationData = {
      ...
      encounterRate: locationGroup.reduce((sum, l) => sum + (l.encounterRate || 0), 0)
    };
...
export const processPokemonEncounters = (species: string | null) => {
      ...
      if (methodData?.mons) {
        ...
        methodData.mons.forEach((mon, slot: number) => {
          if (mon.species === species) {
            ...
            const location = mapLocations.find(([mapId]) => mapId === encounter.map);
